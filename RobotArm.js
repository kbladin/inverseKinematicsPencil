...
RobotArm.prototype.updateAngles = function() {
    ...
    for (var i = 1; i <= this.segmentObjects3D.length; i++) {
	...
	this.endAffector.x = P[this.segmentObjects3D.length].x;
	this.endAffector.y = P[this.segmentObjects3D.length].y;
	this.endAffector.z = P[this.segmentObjects3D.length].z;

	var J = Matrix.Zero(6, robotArm.segmentObjects3D.length);
...
RobotArm.prototype.updatePositions = function() {
    ...
    for (var i = 0; i < this.segmentObjects3D.length; i++) {
		this.segmentObjects3D[i].matrix = new THREE.Matrix4();;
	}
	...
	this.segmentObjects3D[0].applyMatrix(M);
    ...
    for (var i = 1; i < this.segmentObjects3D.length; i++) {
		...
		this.segmentObjects3D[i].applyMatrix(M);
	};
};